...
class Maze{
	...
	horizontalWork(){
		var num_col = this.randomRange(1, this.w-1);
		var num_row = 1;
		...
		num_mid = this.randomRange(start_point[0], start_point[1]);
	...
	verticalWork(){
		var num_row = this.randomRange(1, this.h-1);
		var num_col = 1;
		var start = [-1,-1];
		var work = false;
	...
	corrector(){
		for(var i = 1; i < self.w; i++){
			for(var o = 1; o < self.h;  o++){
				var M = this.maze;
				if (M.getLocal(i, o) == 1){
